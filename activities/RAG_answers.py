...
def build_embedding_store(chunks: Dict[str, str]) -> Dict[str, Any]:
    ...
    embeddings = _embed_texts_fastembed(texts)
    return {
        "chunk_ids": chunk_ids,
        "texts": texts,
...
def retrieve(
    ...
    query_vec = _embed_texts_fastembed([query])[0]
    scores = store["embeddings"] @ query_vec

    k = min(top_k, len(store["chunk_ids"]))
...
def main() -> None:
    ...
    for q in demo_queries:
        ...
        for chunk_id, _, score in hits:
            print(f"{score:.4f}\t{chunk_id}\t{chunks[chunk_id]}")
